...
module.exports = (function() {
    ...
    sql.connect(dbConfig).then(function(output11) {
       var request=new sql.Request();
        ...
        .execute('SP_PRODUCT_CATEGORY_AMD').then(function() {
    ...
    adminCategory.post("/category/y", function (req, res) {
        ...
        transaction.connect(dbConfig).then(function () {      
            transaction.begin().then(function () {
                ...
                request.input("P_SUBCAT_NAME", sql.VarChar(50), req.body.subCatName)
                request.input("P_SUBCAT_DESC", sql.VarChar(100), req.body.subCatAlias)
                request.input("P_USER_ID", sql.VarChar(50), req.body.OPER_TYPE)
                request.input("IS_ACTIVE", sql.Bit, req.body.Active)
                request.input("P_OPER", sql.Char, req.body.OPER_TYPE)
                request.output("FLAG", sql.VarChar(18, 0))
                request.output("MSG", sql.Decimal(18, 0))
                ...
                request.execute("SP_PRODUCT_CATEGORY_AMD").then(function (err,recordsets,returnvalue) {
                    next.ifError(err);
                    transaction.commit().then(function (err,recordsets,returnvalue) {
